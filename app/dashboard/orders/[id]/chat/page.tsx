// app/dashboard/orders/[id]/chat/page.tsx
...
async function sendMessage(formData: FormData) {
  ...
  // Vérifier que la commande existe et que l'utilisateur est bien buyer OU seller
  ...
  if (order.buyerId !== user.id && order.sellerId !== user.id) {
    // Sécurité : pas autorisé à écrire sur une commande qui ne lui appartient pas
    redirect("/dashboard");
  }
  ...
  redirect(`/dashboard/orders/${order.id}/chat`);
}
...
export default async function OrderChatPage({
  ...
  // Vérifier que l'utilisateur est bien l'acheteur ou le vendeur lié
  if (order.buyerId !== user.id && order.sellerId !== user.id) {
    redirect("/dashboard");
  }
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 py-6 space-y-6">
      ...
      <header className="flex flex-col gap-2 border-b border-gray-100 pb-4">
          ...
          <div>
            ...
            {otherParty && (
              <p className="text-[11px] text-gray-500">
                {isBuyer ? "Avec le vendeur" : "Avec l’acheteur"} :{" "}
        ...
        <p className="text-[11px] text-gray-500">
          Utilisez ce chat pour échanger avec{" "}
          {isBuyer ? "le vendeur" : "l’acheteur"} au sujet de cette commande.
          Les échanges restent dans Hemut-link.
        </p>
      </header>
